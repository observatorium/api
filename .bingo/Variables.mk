...
$(GOLANGCI_LINT): $(BINGO_DIR)/golangci-lint.mod
	...
	@cd $(BINGO_DIR) && $(GO) build -mod=mod -modfile=golangci-lint.mod -o=$(GOBIN)/golangci-lint-v1.21.0 "github.com/golangci/golangci-lint/cmd/golangci-lint"

GOYACC := $(GOBIN)/goyacc-v0.1.5
$(GOYACC): $(BINGO_DIR)/goyacc.mod
	@# Install binary/ries using Go 1.14+ build command. This is using bwplotka/bingo-controlled, separate go module with pinned dependencies.
	@echo "(re)installing $(GOBIN)/goyacc-v0.1.5"
	@cd $(BINGO_DIR) && $(GO) build -mod=mod -modfile=goyacc.mod -o=$(GOBIN)/goyacc-v0.1.5 "golang.org/x/tools/cmd/goyacc"
...
$(GUBERNATOR): $(BINGO_DIR)/gubernator.mod
	...
	@cd $(BINGO_DIR) && $(GO) build -mod=mod -modfile=gubernator.mod -o=$(GOBIN)/gubernator-v1.0.0-rc.3 "github.com/mailgun/gubernator/cmd/gubernator"
...
$(OPA): $(BINGO_DIR)/opa.mod
	...
	@cd $(BINGO_DIR) && $(GO) build -mod=mod -modfile=opa.mod -o=$(GOBIN)/opa-v0.23.2 "github.com/open-policy-agent/opa"

PROTOC_GEN_GO := $(GOBIN)/protoc-gen-go-v1.4.2
$(PROTOC_GEN_GO): $(BINGO_DIR)/protoc-gen-go.mod
	@# Install binary/ries using Go 1.14+ build command. This is using bwplotka/bingo-controlled, separate go module with pinned dependencies.
	@echo "(re)installing $(GOBIN)/protoc-gen-go-v1.4.2"
	@cd $(BINGO_DIR) && $(GO) build -mod=mod -modfile=protoc-gen-go.mod -o=$(GOBIN)/protoc-gen-go-v1.4.2 "github.com/golang/protobuf/protoc-gen-go"
...
$(STYX): $(BINGO_DIR)/styx.mod
	...
	@cd $(BINGO_DIR) && $(GO) build -mod=mod -modfile=styx.mod -o=$(GOBIN)/styx-v0.0.0-20200109161911-78a77eb717b4 "github.com/go-pluto/styx"

UP := $(GOBIN)/up-v0.0.0-20200928171403-120d85735d11
$(UP): $(BINGO_DIR)/up.mod
	@# Install binary/ries using Go 1.14+ build command. This is using bwplotka/bingo-controlled, separate go module with pinned dependencies.
	@echo "(re)installing $(GOBIN)/up-v0.0.0-20200928171403-120d85735d11"
	@cd $(BINGO_DIR) && $(GO) build -mod=mod -modfile=up.mod -o=$(GOBIN)/up-v0.0.0-20200928171403-120d85735d11 "github.com/observatorium/up/cmd/up"
