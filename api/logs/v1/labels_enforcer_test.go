...
func TestEnforceValuesOnSeries(t *testing.T) {
	tt := []struct {
		desc           string
		accessMatchers []*labels.Matcher
		urlValues      url.Values
	...
	}{
		{
			...
			urlValues: url.Values{},
			expValues: url.Values{
				"match": []string{`{kubernetes_namespace_name=~"log-test-0"}`},
			},
		},

		{
			...
			expValues: url.Values{
				"match": []string{`{foo="bar", kubernetes_namespace_name=~"log-test-0"}`},
			},
		},
	}
	...
	for _, tc := range tt {
		t.Run(tc.desc, func(t *testing.T) {
			...
			ou, err := url.Parse(fmt.Sprintf("/loki/api/v1/series?%s", tc.urlValues.Encode()))
			testutil.Ok(t, err)
			...
			expr, err := logqlv2.ParseExpr(u.Get("match"))
			testutil.Ok(t, err)

			expected, err := logqlv2.ParseExpr(tc.expValues.Get("match"))
			testutil.Ok(t, err)
