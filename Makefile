...
client/client.gen.go: $(OAPI_CODEGEN) client/spec.yaml
	$(OAPI_CODEGEN) -generate types,client -import-mapping="./parameters.yaml:github.com/observatorium/api/client,./models/models.yaml:github.com/observatorium/api/client/models,./responses/responses.yaml:github.com/observatorium/api/client/responses" -package client client/spec.yaml | sed 's|gopkg.in/yaml.v2|github.com/ghodss/yaml|g' | gofmt -s > $@

client/parameters.gen.go: $(OAPI_CODEGEN) client/parameters.yaml
	$(OAPI_CODEGEN) -generate types,skip-prune -package client client/parameters.yaml | sed 's|gopkg.in/yaml.v2|github.com/ghodss/yaml|g' | gofmt -s > $@
